"""
train_chronos2_lora.py
======================
Chronos-2 LoRA 파인튜닝 & Out-of-Time 실전 추론 평가 스크립트.

Phase 1 (학습):
    train_data.csv를 읽어 LoRA 파인튜닝 수행 → run_YYYYMMDD_HHMMSS/final 에 저장.

Phase 2 (실전 추론 및 검증):
    저장된 모델을 다시 로드 → test_data.csv를 읽어 작물당 1회 14일 예측 수행.
    RMSE / MAE / MAPE(%) 출력 + Matplotlib 그래프 저장.
"""
...
OUTPUT_BASE_DIR = f"./chronos_lora_output/run_{RUN_ID}"
...
def apply_lora_to_model(model):
    print("\n[LoRA] Low-Rank Adaptation 어댑터 부착 중...")
    config = LoraConfig(
...
def train_model(model, raw_list: list[dict]):
    """train_data.csv의 raw_list를 받아 Transformers Trainer로 파인튜닝."""
    print("\n[Phase 1] Transformers Trainer 기반 LoRA 파인튜닝 시작...")
...
# ==========================================
# 4. Phase 2: Out-of-Time 실전 추론 평가
# ==========================================
...
def production_evaluate(pipeline, test_raw_list: list[dict]) -> None:
    ...
    for ts_data in test_raw_list:
        ...
        forecast_arcsinh  = np.median(forecast_obj.numpy()[0], axis=0)  # [14]
        ...
        # ── Matplotlib 그래프 저장 ────────────────────────────────────────
        # 과거 30일 실제 가격(회색) / 미래 14일 실제(초록) / 예측(빨강 점선)
        past_30_arcsinh = full_target[context_length - 30: context_length]
        past_30_real    = np.sinh(past_30_arcsinh)

        fig, ax = plt.subplots(figsize=(12, 5))
        ax.set_title(f"Out-of-Time Forecast: {item_id}", fontsize=13, fontweight="bold")
...
def main():
    print(f"\n{'=' * 60}")
    print(f" Chronos-2 LoRA 파인튜닝 + Out-of-Time 평가")
